...
# 1. Instalacja narzędzia (jeśli nie jest zainstalowane)
...
# 3. Pobieranie zewnętrznych schematów
Write-Host "`nPobieranie zewnętrznych schematów..." -ForegroundColor Green
...
foreach ($schema in $schemasToDownload) {
    if (-not (Test-Path $schema.File)) {
        ...
        try {
            ...
            Write-Host "  UWAGA: Nie można pobrać $($schema.Url)" -ForegroundColor Yellow
            Write-Host "         Sprawdź połączenie lub pobierz ręcznie" -ForegroundColor Yellow
        }
    }
    else {
        Write-Host "  Używam z cache: $($schema.File)" -ForegroundColor Gray
    }
}

# 4. Kopiowanie głównego pliku CEIDG
Copy-Item "CEIDG-1_10122023.xsd" "$cacheDir/CEIDG-1_10122023.xsd" -Force

# 5. Modyfikacja głównego pliku - zmiana schemaLocation na lokalne ścieżki
Write-Host "`nModyfikowanie schemaLocation na lokalne ścieżki..." -ForegroundColor Green
...
# 6. Przygotowanie mapowania namespace'ów
Write-Host "`nPrzygotowywanie mapowania namespace'ów..." -ForegroundColor Green
$namespaceMapping = @"
...
# Opcja A: Z mapowaniem namespace'ów (zalecane)
$generateCommand = @"
xscgen `
    --nf "$cacheDir/namespaces.txt" `
    --separateFiles `
    --output "$outputDir" `
    --nullable `
    --netCore `
...
# Alternatywnie: jeśli występują konflikty nazw, dodaj --uniqueTypeNames
# $generateCommandWithUniqueNames = @"
# xscgen `
#     --nf "$cacheDir/namespaces.txt" `
#     --separateFiles `
#     --output "$outputDir" `
...
if (Test-Path $outputDir) {
    ...
    Write-Host "Wygenerowano $($generatedFiles.Count) plików C#" -ForegroundColor Cyan
    Write-Host "Lokalizacja: $outputDir" -ForegroundColor Cyan
    
    Write-Host "`nStruktura katalogów:" -ForegroundColor Yellow
    Get-ChildItem -Path $outputDir -Directory -Recurse | ForEach-Object {
        Write-Host "  📁 $($_.FullName.Replace($PWD, '.'))" -ForegroundColor Gray
    }
    
    Write-Host "`nPrzykładowe pliki:" -ForegroundColor Yellow
    $generatedFiles | Select-Object -First 10 | ForEach-Object {
        Write-Host "  📄 $($_.FullName.Replace($PWD, '.'))" -ForegroundColor Gray
    }
}
else {
    Write-Host "BŁĄD: Nie udało się wygenerować klas" -ForegroundColor Red
}

Write-Host "`n=== UŻYCIE W PROJEKCIE ===" -ForegroundColor Green
Write-Host @"
